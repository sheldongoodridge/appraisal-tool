...
export const acceptInvite = async (token, data) => {
  const response = await axios.post(`${API_URL}/invite/${token}/accept`, data);
  return response.data;
};

// Workfile settings (file number sequences)
...
// Workfile status / billing updates
export const updateWorkflowStatus = async (id, workflow_status) => {
  const response = await apiClient.patch(`/inspections/${id}/status`, { workflow_status });
  return response.data;
};

export const updateBillingStatus = async (id, billing_status) => {
  const response = await apiClient.patch(`/inspections/${id}/billing`, { billing_status });
  return response.data;
};

export const updateFileNumbers = async (id, data) => {
  const response = await apiClient.patch(`/inspections/${id}/file-numbers`, data);
  return response.data;
};
