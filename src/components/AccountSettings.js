...
export default function AccountSettings({ currentUser, onUserUpdate }) {
  ...
  const handleSaveFileNumbering = async () => {
    ...
    try {
      const saved = await saveWorkfileSettings({
        prefix: fnPrefix, current_number: fnNumber,
        year_format: fnYearFormat, separator: fnSeparator, suffix: fnSuffix,
      });
      setFileSeq(saved);
      setFnMsg('Saved!');
      setTimeout(() => setFnMsg(''), 2500);
    } catch (err) {
      setFnMsg('Failed to save.');
    } finally {
      setFnSaving(false);
    }
  };
  ...
  const copyInviteLink = (token) => {
    const link = `${window.location.origin}/appraisal-tool?invite=${token}`;
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div className="section-content">
          {isOrgAdmin ? (
            ...
            <>
              ...
              <p><strong>Billing Email:</strong> {orgData?.billing_email || '—'}</p>
            </>
          )}
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                ...
                {isOrgAdmin && <th style={{ padding: '8px 4px' }}></th>}
              </tr>
            </thead>
            <tbody>
              {members.map(member => (
                <tr key={member.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  <td style={{ padding: '10px 4px' }}>
                    <span style={{ color: member.is_active ? '#22c55e' : '#ef4444', fontWeight: 500 }}>
                    ...
                    <td style={{ padding: '10px 4px', textAlign: 'right' }}>
                      {member.email !== currentUser?.email && (
                        <button
                          ...
                          onClick={() => {
                            if (window.confirm(`Remove ${member.full_name} from your organization?`)) {
                              // Future: call remove API
                              alert('Member removal coming soon.');
                            }
                          }}
                        >
                          Remove
      ...
      {/* Section 4: File Numbering */}
      <div className="form-section">
        <div className="section-header" style={{ cursor: 'default' }}>
          <h2>File Numbering</h2>
        </div>
        <div className="section-content">
          <p style={{ fontSize: 13, color: '#6b7280', marginTop: 0, marginBottom: 16 }}>
            Configure how your appraiser file numbers are generated for new workfiles.
          </p>
          ...
          <div className="fn-grid">
            <div className="form-group">
              ...
              <input
                type="number" min="0" value={fnNumber}
                onChange={e => setFnNumber(parseInt(e.target.value) || 0)}
              />
              <small style={{ color: '#6b7280', fontSize: 12 }}>Next number to assign (auto-increments)</small>
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              <label>Suffix (initials)</label>
              <input type="text" value={fnSuffix} onChange={e => setFnSuffix(e.target.value)} placeholder="e.g. GD" maxLength={10} />
            </div>
            <div className="form-group">
              <label>Prefix <span style={{ fontWeight: 400, color: '#9ca3af' }}>(optional)</span></label>
              <input type="text" value={fnPrefix} onChange={e => setFnPrefix(e.target.value)} placeholder="e.g. ON-" maxLength={10} />
            </div>
          </div>

          <button className="btn btn-primary" onClick={handleSaveFileNumbering} disabled={fnSaving} style={{ marginTop: 8 }}>
            {fnSaving ? 'Saving…' : 'Save Format'}
          </button>
        ...
        <div className="section-content">
          <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ fontWeight: 600, fontSize: 16 }}>{PLAN_LABELS[orgData?.plan_type] || orgData?.plan_type}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 2 }}>Appraisers</div>
              <div style={{ fontWeight: 600, fontSize: 16 }}>
                {members.filter(m => m.role === 'appraiser' || m.role === 'org_admin').length} / {orgData?.max_appraisers}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 2 }}>Assistants</div>
              <div style={{ fontWeight: 600, fontSize: 16 }}>
                {members.filter(m => m.role === 'assistant').length} / {orgData?.max_assistants}
              </div>
            </div>
          </div>
          {isOrgAdmin && (
            <button className="btn btn-small" disabled style={{ opacity: 0.5 }}>
